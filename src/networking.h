...
// notes: remote files can use ofImage.loadImage("http://some.com/url.png")
	...
	static const int16_t maxSend = 1024 * 1024;
	...
	// ex: ofTexture::readToPixels(pixels); // now all the pixels from tex are in pix
		float width;//bugbug tie back to kinect = getDepthFrameWidth();
		float height;//bugbug tie back to kinect = getDepthFrameHeight();
	protected:
		void update();
