...
bool compress(const char*buffer, size_t len, string&output) {
	...
	//bugbug add snappy and ssl of some kind size_t size = snappy::Compress((const char*)buffer, len, &output);
	//bugbugif (size <= 0) {
	//bugbug	ofLogError("compress") << "fails " << size;
	//bugbug}
	//bugbugreturn size > 0;
}
...
bool uncompress(const char*buffer, size_t len, string&output) {
	...
	output.append(buffer, len);
	return true;
///	bugbug add compress/ssl(in of) if (!snappy::Uncompress(buffer, len, &output)) {
